 ...
 * stable values.
 * 
 * Modified to expose the accumulator's current center value.
 * @author James
 */
public class FalconAnalogChannel extends SensorBase implements PIDSource,
		LiveWindowSendable {
	 ...
	 */
	public FalconAnalogChannel(final int channel) {
	 ...
	 *            The channel number to represent.
	 */
	public FalconAnalogChannel(final int moduleNumber, final int channel) {
		...
		UsageReporting.report(UsageReporting.kResourceType_AnalogChannel,
	...
	/**
	 * Gets the accumulator center value.
	 * 
	 * The center value is subtracted from each A/D value before it is added to
	 * the accumulator. This is used for the center value of devices like gyros
	 * and accelerometers to make integration work and to take the device offset
	 * into account when integrating.
	 * 
	 * @return the current center value of the accumulator
	 */
	public int getAccumulatorCenter() {
		return m_accumulator.readCenter();
	}
	...
	/**
	 * Set whether to use voltage of value for PIDGet This method determines
	 * whether PIDGet uses average voltage or value for PID controllers for a
	 * particular channel. This is to preserve compatibility with existing
	 * programs and is likely to change in favor of voltage for RoboRIO.
	 * 
	 * @param m_shouldUseVoltageForPID
	 *            True if voltage should be used for PIDGet. The default is to
	 *            use the value as it has been since the creation of the
	 *            library.
	 */
	...
	/**
	 * Get the average value for use with PIDController. This can be changed to
	 * use average voltage by calling setVoltageForPID().
	 * 
