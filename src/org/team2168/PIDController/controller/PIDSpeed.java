...
public class PIDSpeed implements TCPMessageInterface {
	...
	public synchronized void receiveJSON(String[] message) {
		...
		try {
			...
			// // Enable();
			// //new
			// PID_SetAftWheelSpeed(Double.valueOf(message[3]).doubleValue()).start();
			//
			// }//new
			// PID_Drive(Double.valueOf(message[3]).doubleValue()).start();
			// else
	...
	private synchronized void calculate() {
		...
		if (enable) {
			...
			// rate to its last
			// value.
			double tempRate = encoder.getRate();// cp is in units distance per
												// second (i.e
												// inches/sec)
